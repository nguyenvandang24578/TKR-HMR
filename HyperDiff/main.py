...
# dataset_path = '/mnt/ssd1/han/code/data/data_3d_' + args.dataset + '.npz' # Gemini
dataset_path = '/mnt/hdd2/han/code/data/data_3d_' + args.dataset + '.npz' # Uranus
# dataset_path = '/mnt/hdd/Han/code/data/data_3d_' + args.dataset + '.npz'  # Jupiter
...
keypoints = np.load('/mnt/hdd2/han/code/data/data_2d_' + args.dataset + '_' + args.keypoints + '.npz', allow_pickle=True)
...
model_pos_train = MY_frame(args, joints_left, joints_right, is_train=True)
model_pos_test_temp = MY_frame(args,joints_left, joints_right, is_train=False)
model_pos = MY_frame(args,joints_left, joints_right,  is_train=False, num_proposals=args.num_proposals, sampling_timesteps=args.sampling_timesteps)
...
inputs_2d_first = torch.rand(1,1,17,2).cuda()
inputs_3d_first = torch.rand(1,1,17,3).cuda()
device = torch.device('cuda:0')
from fvcore.nn import FlopCountAnalysis
flop_analysis = FlopCountAnalysis(model_pos, inputs=(inputs_2d_first,inputs_3d_first,inputs_2d_first))
print(f"FLOPs: {flop_analysis.total()/1000000000}") 
...
if not args.evaluate:
    ...
    while epoch < args.epochs:
        ...
        for cameras_train, batch_3d, batch_2d in train_generator.next_epoch():
            ...
            # print('predicted_3d_pose shape:', predicted_3d_pos.shape)
...
def evaluate(test_generator, action=None, return_predictions=False, use_trajectory_model=False, newmodel=None):
    ...
    elapsed_time = Num / elapsed_sum
    elapsed_all += elapsed_sum
    print('my testing fps',elapsed_time)
...
if args.render:
    ...
    def run_evaluation(actions, action_filter=None):
        ...
        fps_all = 543344 / elapsed_all
        print('my testing fps all frame ',fps_all)
        log_path = os.path.join(args.checkpoint, 'h36m_test_log_H%d_K%d.txt' %(args.num_proposals, args.sampling_timesteps))
        f = open(log_path, mode='a')
        for ii in range(errors_p1_actionwise.shape[0]):
            ...
            if args.p2:
                ...
                f.write('step %d Protocol #2   (MPJPE) action-wise average J_Agg: %f mm\n' % (
                    ii, errors_p2_actionwise_select[ii].item()))
        f.write('my testing fps all frame %d \n ' % (fps_all))
        f.close()
