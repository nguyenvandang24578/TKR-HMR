...
class Human36M(torch.utils.data.Dataset):
    def __init__(self, mode, args):
        ...
        elif cfg.MODEL.name in ['ARTS', 'TKR', 'Teacher', 'Student']:
    ...
    def get_single_item(self, idx):
        ...
        for num in range(self.seqlen):
            ...
            if cfg.MODEL.name in ['TKR', 'Teacher', 'Student']:
            ...
            if cfg.MODEL.name in ['PoseEst', 'Teacher', 'Student']:
                posenet_joint_cam.append(joint_cam.reshape(1, len(joint_cam), 3))
                valid = np.ones((len(joint_img), 1), dtype=np.float32)
                joint_valid.append(valid.reshape(1, len(joint_img), 1))
    ...
    def evaluate_joint(self, outs):
        ...
        for n in range(sample_num):
            ...
            pose_coord_out = pose_coord_out[self.seqlen //2 ,:,:]
