...
def prepare_network(args, load_dir='', is_train=True):
    ...
    if is_train or load_dir:
        ...
        if cfg.MODEL.name == 'Teacher':
            model = models.TeacherFusion.get_model(embed_dim=cfg.MODEL.hpe_dim)
...
class Trainer:
    def __init__(self, args, load_dir):
        ...
        if cfg.TRAIN.wandb:
            wandb.init(config=cfg,
                   ...
                   name='Teacher/' + cfg.output_dir.split('/')[-1],
                   dir=cfg.output_dir,
                   job_type="training",
                   reinit=True)
    ...
    def train(self, epoch):
        ...
        for i, (inputs, targets, meta) in enumerate(batch_generator):
            ...
            # Khởi tạo loss1, loss2, loss4 (giống hệt base.py)
            ...
            # Tính SMPL loss và Laplacian loss
