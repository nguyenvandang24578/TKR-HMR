...
def construct_H_with_KNN(X, K_neigs=[10], split_diff_scale=False, is_probH=True, m_prob=1):
    ...
    for k_neig in K_neigs:
        ...
        if not split_diff_scale:
            ...
            H.append(H_tmp)
    return H
# 2. Định nghĩa ma trận H dựa trên cấu trúc cơ thể
...
def build_human_H_matrix():
    ...
    torso_joints     = [0, 3, 6, 9, 12, 15]
    left_leg_joints  = [0, 1, 4, 7, 10]       # thêm joint 0 (pelvis) làm gốc
    right_leg_joints = [0, 2, 5, 8, 11]       # thêm joint 0 (pelvis) làm gốc
    left_arm_joints  = [9, 13, 16, 18, 20, 22]  # thêm joint 9 (spine3) làm gốc
    right_arm_joints = [9, 14, 17, 19, 21, 23]  # thêm joint 9 (spine3) làm gốc
